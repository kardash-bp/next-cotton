...
const AddToCartBtn = ({
  ...
  const [createCartItem, { error: addError, loading: addLoading }] =
    useMutation(ADD_TO_CART_MUTATION, {
      variables: { quantity: '1', pid, price, uid: user.id, email: user.email },
    })
  ...
  const handleClick = async () => {
    ...
    if (!productInCart) {
      ...
      const newCart = [...cartProducts, res.data.createCartItem]
      setCart(newCart)
    } else {
      console.log(productInCart, user)
      ...
      console.log({ res })
      const newCart = [...cartProducts, res.data.updateCartItem]
      setCart(newCart)
    }
  }
  return (
